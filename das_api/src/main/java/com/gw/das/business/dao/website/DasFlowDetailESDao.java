 ...
 */
@Repository
public class DasFlowDetailESDao {
	 ...
	 * @throws Exception
	 */
	public PageGrid findDasFlowDetailPageList(DasFlowDetailSearchBean model)
			throws Exception {
		...
		SearchRequestBuilder srb = client.prepareSearch("das_web_detail_index");
		//设置查询类型  
		...
		if(StringUtils.isNotBlank(model.getPlatformType())){//访问客户端
		...
		List<DasFlowDetailES> list = new ArrayList<DasFlowDetailES>();
		for (SearchHit searchHit : hits) {
			Map<String, Object> source = searchHit.getSource();								
			DasFlowDetailES entity = JacksonUtil.readValue(JacksonUtil.toJSon(source), DasFlowDetailES.class);
     ...
     * @throws Exception
     */
	public List<DasFlowDetailES> findDasFlowDetailList(DasFlowDetailSearchBean model)
			throws Exception {
		SearchRequestBuilder srb = client.prepareSearch("das_web_detail_index");
	 ...
	 * @throws Exception
	 */
	public PageGrid findDasflowUrlDetailPageList(DasFlowDetailUrl model)
			throws Exception {
		int pageNo = model.getPageNumber();
		int pageSize = model.getPageSize();
		PageGrid page = new PageGrid();
		page.setPageNumber(pageNo);
		page.setPageSize(pageSize);
		SearchRequestBuilder srb = client.prepareSearch("das_web_url_detail_index");
		//设置查询类型  
		srb.setSearchType(SearchType.DFS_QUERY_THEN_FETCH);
		if(BusinessPlatformEnum.Fx.getLabelKey().equals(model.getBusinessPlatform())){
			srb.setTypes("fx");
		...
		if(StringUtils.isNotBlank(model.getPlatformType()) && !"null".equals(model.getPlatformType())){//访问客户端
        	query.must(QueryBuilders.matchPhraseQuery("platformType", model.getPlatformType()));
        }
		...
		List<DasFlowDetailUrl> list = new ArrayList<DasFlowDetailUrl>();
		for (SearchHit searchHit : hits) {
			Map<String, Object> source = searchHit.getSource();
			DasFlowDetailUrl entity = JacksonUtil.readValue(JacksonUtil.toJSon(source), DasFlowDetailUrl.class);			
			list.add(entity);
		}
		page.setRows(list);
		return page;
	 ...
	 * @throws Exception
	 */
	public List<DasFlowDetailUrl> findDasflowUrlDetailESList(DasFlowDetailUrl model)
			throws Exception {
		SearchRequestBuilder srb = client.prepareSearch("das_web_url_detail_index");
		//设置查询类型  
		srb.setSearchType(SearchType.DFS_QUERY_THEN_FETCH);
