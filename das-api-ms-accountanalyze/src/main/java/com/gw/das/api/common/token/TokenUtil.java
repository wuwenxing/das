...
import com.gw.das.api.common.utils.HttpUtil;
import com.gw.das.api.common.utils.JacksonUtil;
import com.gw.das.api.common.utils.SystemConfigUtil;
...
public class TokenUtil {
	...
	static {
		...
		logger.debug("==>>static[serviceScretUrl={},serviceScretType={}]", serviceScretUrl, serviceScretType);
	}
	...
	public static TokenResult getToken(String appId, String appScret) {
		...
		try {
			...
			logger.debug("==>>getToken[jsonStr={}]", jsonStr);
			result = JacksonUtil.readValue(jsonStr, TokenResult.class);
		} catch (IOException e) {
			logger.debug("==>>getToken error!", e);
		}
		return result;
	}
	...
	public static VerifyTokenResult checkToken(String interfaceUrl, String token) {
		...
		try {
			...
			logger.debug("==>>checkToken[jsonStr={}]", jsonStr);
			result = JacksonUtil.readValue(jsonStr, VerifyTokenResult.class);
		} catch (IOException e) {
			logger.debug("==>>checkToken error!", e);
		}
		return result;
	}
