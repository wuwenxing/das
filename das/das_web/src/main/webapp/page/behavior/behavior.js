...
var behavior = {
	...
	loadDataGrid : function(){
		// 初始参数
		var url = BASE_PATH + 'DasUserBehaviorController/pageList';
		var queryParams = {
		};
	 			...
	 			{field : 'visitCount', title : '页面浏览数', sortable : false, width : 50, 
	 				formatter : function(value, rowData, rowIndex) {
	 					if(rowData.visitCount > 0){
	 						return "<a onclick=\"behavior.viewUrl('"+rowData.rowKey+"', '" + rowData.formatTime + "', '" + $("#platformTypeSearch").datebox('getValue') + "', '" + $("#utmcmdSearch").val() + "', '" + $("#utmcsrSearch").val() + "')\">" + rowData.visitCount + "</a>";	
	 					}else{
	...
	exportExcel : function(){
		...
		var url = BASE_PATH + "DasUserBehaviorController/exportExcel?";
	 ...
	 * 浏览详细
	 */
	viewUrl : function(flowDetailId, formatTime,platformType,utmcmdh,utmcsr){
		var url = "DasFlowDetailUrlController/page?flowDetailId=" + flowDetailId + "&formatTime=" + formatTime
