$(function() {
	statisticsVisitor.init();
});

var statisticsVisitor = {
	dataGridId: "dataGrid",
	 ...
	 */
	init:function(){
		statisticsVisitor.loadDataGrid();
	},
	/**
	 * 加载dataGrid
	 */
	loadDataGrid : function(){
		// 初始参数
		var url = BASE_PATH + 'DasChartRoomController/findStatisticsByUserTypePage/2';
		var queryParams = {
		};
		...
		$('#' + statisticsVisitor.dataGridId).datagrid(easyui.defaultOption).datagrid({
			url : url,
			...
			onLoadSuccess : function(data){
				...
				if($("#courseNameChecked").val() == 'true'){
					$("#" + statisticsVisitor.dataGridId).datagrid('showColumn', 'courseName');
				}else{
					$("#" + statisticsVisitor.dataGridId).datagrid('hideColumn', 'courseName');
				}
				if($("#teacherNameChecked").val() == 'true'){
					$("#" + statisticsVisitor.dataGridId).datagrid('showColumn', 'teacherName');
				}else{
					$("#" + statisticsVisitor.dataGridId).datagrid('hideColumn', 'teacherName');
				}
				if($("#roomNameChecked").val() == 'true'){
					$("#" + statisticsVisitor.dataGridId).datagrid('showColumn', 'roomName');
				}else{
					$("#" + statisticsVisitor.dataGridId).datagrid('hideColumn', 'roomName');
				}
				var reportType = $("#reportTypeSearch").combobox('getValue');
				if(reportType == 'days' || reportType == 'months'){
					$("#" + statisticsVisitor.dataGridId).datagrid('hideColumn', 'weeks');
				}
				if(reportType == 'weeks'){
					$("#" + statisticsVisitor.dataGridId).datagrid('showColumn', 'weeks');
				}
				statisticsVisitorChart.loadChartData(1);
				common.iFrameHeight();
			}
		});
	},
	getQueryParams: function() {
		var queryParams = $("#" + statisticsVisitor.dataGridId).datagrid('options').queryParams;
		queryParams['reportType'] = $("#reportTypeSearch").combobox('getValue');
	 ...
	 */
	find: function(){
		statisticsVisitor.getQueryParams();
		common.loadGrid(statisticsVisitor.dataGridId);
	},
	/**
	 * reset条件查询
	 */
	reset: function(){
		$('#' + statisticsVisitor.searchFormId).form('reset');
	...
	exportExcel : function(){
		...
		var queryParams = statisticsVisitor.getQueryParams();
		queryParams['userTypeStatistics'] = 2;
