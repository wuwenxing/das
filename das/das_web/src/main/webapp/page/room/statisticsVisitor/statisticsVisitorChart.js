...
var statisticsVisitorChart = {
	...
	findChartData : function() {
		...
		if (type == 1) {
			tab.panel('refresh', 'page/room/statisticsVisitor/statisticsVisitorChart.html');
		}
	},
	/**
	 * 获取图表数据
	...
	loadChartData : function(type) {
		statisticsVisitorChart.showOrHideLoadingDiv(type, true);
		
		// 获取表格数据
		var rows = $('#' + statisticsVisitor.dataGridId).datagrid('getRows');
		...
		// 显示图表
		statisticsVisitorChart.showOrHideLoadingDiv(type, false);
		if (type == 1) {
			statisticsVisitorChart.showChart1(dateTimeArray, vipNumbersArray, realANumbersArray, realNNumbersArray
		...
		var	option = {
		    ...
		    toolbox: {
		        ...
		        feature: {
		            ...
		            restore: {show: false},
		            saveAsImage: {show: true,name:"直播间访问数据"}
		        }
		    },
		    legend: {
		        width: '450',
		        x : 'center',
		    	data:['VIP用户','真实A用户','真实N用户','模拟用户','注册用户','游客','分析师','管理员','客服','总次数']
		    },
		    xAxis: [
		        {
		            type: 'category',
