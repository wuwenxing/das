...
var statisticsSpeakChart = {
	...
	findChartData : function() {
		...
		if (type == 1) {
			tab.panel('refresh', 'page/room/statisticsSpeak/statisticsSpeakChart.html');
		}
	},
	/**
	 * 获取图表数据
	...
	loadChartData : function(type) {
		statisticsSpeakChart.showOrHideLoadingDiv(type, true);
		
		// 获取表格数据
		var rows = $('#' + statisticsSpeak.dataGridId).datagrid('getRows');
		...
		// 显示图表
		statisticsSpeakChart.showOrHideLoadingDiv(type, false);
		if (type == 1) {
			statisticsSpeakChart.showChart1(dateTimeArray, vipNumbersArray, realANumbersArray, realNNumbersArray
		...
		var	option = {
		    ...
		    toolbox: {
		        ...
		        feature: {
		            ...
		            restore: {show: false},
		            saveAsImage: {show: true,name:"直播间发言数据"}
		        }
		    },
		    legend: {
		        width: '450',
