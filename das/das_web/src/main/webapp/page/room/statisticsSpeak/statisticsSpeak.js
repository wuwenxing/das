...
var statisticsSpeak = {
	...
	loadDataGrid : function(){
		// 初始参数
		var url = BASE_PATH + 'DasChartRoomController/findStatisticsByUserTypePage/3';
		var queryParams = {
		};
		queryParams['reportType'] = $("#reportTypeSearch").combobox('getValue');
		queryParams['speakType'] = $("#speakTypeSearch").combobox('getValue');
		...
		$('#' + statisticsSpeak.dataGridId).datagrid(easyui.defaultOption).datagrid({
			url : url,
			...
			onLoadSuccess : function(data){
				...
				if($("#courseNameChecked").val() == 'true'){
					$("#" + statisticsSpeak.dataGridId).datagrid('showColumn', 'courseName');
				}else{
					$("#" + statisticsSpeak.dataGridId).datagrid('hideColumn', 'courseName');
				}
				if($("#teacherNameChecked").val() == 'true'){
					$("#" + statisticsSpeak.dataGridId).datagrid('showColumn', 'teacherName');
				}else{
					$("#" + statisticsSpeak.dataGridId).datagrid('hideColumn', 'teacherName');
				}
				if($("#roomNameChecked").val() == 'true'){
					$("#" + statisticsSpeak.dataGridId).datagrid('showColumn', 'roomName');
				}else{
					$("#" + statisticsSpeak.dataGridId).datagrid('hideColumn', 'roomName');
				}

				var reportType = $("#reportTypeSearch").combobox('getValue');
				if(reportType == 'days' || reportType == 'months'){
					$("#" + statisticsSpeak.dataGridId).datagrid('hideColumn', 'weeks');
				}
				if(reportType == 'weeks'){
					$("#" + statisticsSpeak.dataGridId).datagrid('showColumn', 'weeks');
				}
				statisticsSpeakChart.loadChartData(1);
				common.iFrameHeight();
			}
		});
	},
	getQueryParams: function() {
		var queryParams = $("#" + statisticsSpeak.dataGridId).datagrid('options').queryParams;
		queryParams['reportType'] = $("#reportTypeSearch").combobox('getValue');
	 ...
	 */
	find: function(){
		statisticsSpeak.getQueryParams();
		common.loadGrid(statisticsSpeak.dataGridId);
	},
	/**
	 * reset条件查询
	 */
	reset: function(){
		$('#' + statisticsSpeak.searchFormId).form('reset');
	...
	exportExcel : function(){
		...
		var queryParams = statisticsSpeak.getQueryParams();
		queryParams['userTypeStatistics'] = 3;
