 ...
 */
@Service
public class SmsFcyListenerImpl implements SmsListener{

	private static final Logger logger = LoggerFactory.getLogger(SmsFcyListenerImpl.class);
	...
	public void updateBefore(SmsContext smsContext) {
		try{
			...
			logger.info("fcy_sms-before[phones=" + smsInfo.getPhones() + "]");
...
//			this.updateTotal(smsInfo.getTemplateId(), totalNum, "totalNum");
	...
	public void updateAfter(SmsContext smsContext) {
		try{
			SmsInfo smsInfo = smsContext.getSmsInfo();
			logger.info("fcy_sms-after[phones=" + smsInfo.getPhones() + "]");
			...
			int loseNum = 0;
			if (null != response) {
				logger.error("fcy_sms-返回resCode：" + response.getResCode());
				...
				loseNum = phoneAry.length;
				logger.error("fcy_sms-请求异常response=null");
...
//			this.updateTotal(smsInfo.getTemplateId(), loseNum, "failNum");
	...
	public void updateAfterThrowable(SmsContext smsContext) {
		try{
			SmsInfo smsInfo = smsContext.getSmsInfo();
			logger.info("fcy_sms-afterThrowable[phones=" + smsInfo.getPhones() + "]");
...
//		synchronized (SmsFcyListenerImpl.class) {
