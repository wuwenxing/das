 ...
 */
@Service
public class SmsZqhlListenerImpl implements SmsListener {

	private static final Logger logger = LoggerFactory.getLogger(SmsZqhlListenerImpl.class);
	...
	public void updateBefore(SmsContext smsContext) {
		try{
			...
			logger.info("zqhl_sms-before[phones=" + smsInfo.getPhones() + "]");
...
//			this.updateTotal(smsInfo.getTemplateId(), totalNum, "totalNum");
	...
	public void updateAfter(SmsContext smsContext) {
		try{
			SmsInfo smsInfo = smsContext.getSmsInfo();
			logger.info("zqhl_sms-after[phones=" + smsInfo.getPhones() + "]");
			...
			int loseNum = 0;
			if (null != response) {
				logger.error("zqhl_sms-返回resCode：" + response.getResCode());
				...
				if ("00".equals(response.getResCode()) || "03".equals(response.getResCode())) {
					...
					for (int i = 0; i < phoneAry.length; i++) {
						...
						detailVo.setSmsid(response.getResId()==null?"":response.getResId().trim());
				...
				loseNum = phoneAry.length;
				logger.error("zqhl_sms-请求异常response=null");
				for (int i = 0; i < phoneAry.length; i++) {
					String tempMobile = phoneAry[i];
					SmsDetailEntity detailVo = tempRecordMap.get(tempMobile);
					detailVo.setCommitStatus(CommitStatusEnum.commitSuccess.getLabelKey());// 短信提交状态
...
//			this.updateTotal(smsInfo.getTemplateId(), loseNum, "failNum");
	...
	public void updateAfterThrowable(SmsContext smsContext) {
		try{
			SmsInfo smsInfo = smsContext.getSmsInfo();
			logger.info("zqhl_sms-afterThrowable[phones=" + smsInfo.getPhones() + "]");
...
//		synchronized (SmsZqhlListenerImpl.class) {
